...
for fold = 1:folds
    ...
    itr = 100 + ((size(train_data,1)-1.6*size(trpf))/size(trpf))*20;
    
    gen_sample = [];
    % Generating samples using Gibbs Sampler
    for i = 1:size(train_pos,1)
        disp(['     Generating new minority class sample: ' int2str(i)]);
        sample = GibbsSampler(train_pos(:,1:end-1),train_pos(i,1:end-1),tan,itr);  
        gen_sample = [gen_sample; sample];
    end
    gen_sample = [gen_sample ones(size(gen_sample,1),1)];
    new_data = [gen_sample; train_data];
